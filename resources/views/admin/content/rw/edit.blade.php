...
<div class="container mx-auto p-4">
  ...
  <div class="bg-white shadow-lg rounded-2xl p-6">
    ...
    <form action="{{ route('rw.update', $rw->id) }}" method="POST" class="space-y-5">
      @csrf @method('PUT')
      <div>
        <label class="block font-medium text-gray-700">Nomor RW</label>
        <input type="text" name="nomor_rw" value="{{ old('nomor_rw', $rw->nomor_rw) }}" required
      ...
      <div>
        ...
        <select name="id_dusun" required
          ...
          @foreach ($dusuns as $dusun)
            <option value="{{ $dusun->id }}" {{ (old('id_dusun', $rw->dusun_id) == $dusun->id) ? 'selected' : '' }}>
              {{ $dusun->nama_dusun }}
            </option>
          @endforeach
        </select>
