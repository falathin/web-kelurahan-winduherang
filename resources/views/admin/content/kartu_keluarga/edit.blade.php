...
<div class="container mx-auto p-6">
  ...
  <div class="bg-white shadow-lg rounded-lg p-6 mt-4" data-aos="fade-up">
    <h2 class="text-2xl font-bold text-green-800 mb-4">Form Ubah Kartu Keluarga</h2>
    ...
    <form action="{{ route('kk.update', $kartuKeluarga) }}" method="POST" class="space-y-4" data-aos="fade-up" data-aos-delay="200">
      @csrf @method('PUT')

      <div>
        <label class="block font-medium text-gray-700">No KK</label>
        <input type="text" name="no_kk" value="{{ old('no_kk', $kartuKeluarga->no_kk) }}"
      ...
      <div>
        ...
        <input type="text" name="kepala_keluarga" value="{{ old('kepala_keluarga', $kartuKeluarga->kepala_keluarga) }}"
               class="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-green-200" required>
      </div>
      ...
      <div>
        ...
        <textarea name="alamat" rows="3"
                  ...
                  required>{{ old('alamat', $kartuKeluarga->alamat) }}</textarea>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label class="block font-medium text-gray-700">RT</label>
          <input type="text" name="id_rt" value="{{ old('id_rt', $kartuKeluarga->id_rt) }}"
                 class="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-green-200" required>
        </div>
        <div>
          <label class="block font-medium text-gray-700">RW</label>
          <input type="text" name="id_rw" value="{{ old('id_rw', $kartuKeluarga->id_rw) }}"
                 class="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-green-200" required>
        </div>
        <div>
          <label class="block font-medium text-gray-700">Dusun</label>
          <input type="text" name="id_dusun" value="{{ old('id_dusun', $kartuKeluarga->id_dusun) }}"
                 class="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-green-200" required>
        </div>
      </div>
