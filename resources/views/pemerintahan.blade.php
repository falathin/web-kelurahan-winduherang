...
@section('content')
    ...
    <div class="max-w-7xl mx-auto px-4 py-16 space-y-24">
        ...
        <section class="space-y-6">
            ...
            <div class="flex flex-col md:flex-row gap-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 flex-1">
                    @foreach ([['role' => 'Ketua', 'name' => 'Ahmad Sukardi'], ['role' => 'Wakil Ketua', 'name' => 'Siti Marlina'], ['role' => 'Sekretaris', 'name' => 'Budi Santoso'], ['role' => 'Bendahara', 'name' => 'Dewi Kartika']] as $m)
        ...
        <section class="space-y-6">
            ...
            <p class="text-gray-700 leading-relaxed">
                Karang Taruna mempersatukan pemuda desa untuk kegiatan sosial, budaya, dan olahraga melalui pelatihan dan
                bakti sosial.
            </p>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
                @foreach ([['Pembina', 'H. Kusnadi'], ['Ketua', 'Rina Wulandari'], ['Sekretaris', 'Taufik Hidayat'], ['Bendahara', 'Indah Permata']] as $m)
        ...
        <section class="space-y-6">
            ...
            <div class="flex flex-col md:flex-row gap-6">
                <img src="https://picsum.photos/seed/posyandu/400/250" alt="Struktur Posyandu"
                    class="rounded-lg shadow-lg object-cover w-full md:w-1/3">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 flex-1">
                    @foreach ([['Kader Utama', 'Sari Melati'], ['Kader Pendamping', 'Dewi Anggraini'], ['Sekretaris', 'Rudi Hartono']] as $m)
                        <div class="bg-white p-4 rounded-lg shadow hover:shadow-xl transition">
                            <p class="font-semibold">{{ $m[0] }}</p>
                            <p class="text-green-700">{{ $m[1] }}</p>
                        </div>
        ...
        <section class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
                @foreach ([['Ketua', 'Yulia Rahma'], ['Wakil', 'Maya Sari'], ['Sekretaris', 'Nur Aisyah'], ['Bendahara', 'Citra Lestari']] as $m)
                    <div class="bg-white p-6 rounded-lg shadow hover:shadow-lg transition text-center">
                        <p class="font-semibold">{{ $m[0] }}</p>
                        <p class="text-green-700">{{ $m[1] }}</p>
                    </div>
        ...
        <section class="space-y-6">
            ...
            @php
                use App\Models\Rt;
                // Mengambil semua RT beserta relasi RW-nya
                $rts = Rt::with('rw')->get();
            @endphp
                    ...
                    <tbody>
                        @foreach ($rts as $rt)
                            <tr class="{{ $loop->odd ? 'bg-white' : 'bg-green-50' }}">
                                {{-- Format nomor 2 digit --}}
                                <td class="px-4 py-2">RT {{ sprintf('%02d', $rt->nomor_rt) }}</td>
                                {{-- Kolom ketua_rt di tabel rts --}}
                                <td class="px-4 py-2">{{ $rt->ketua_rt }}</td>
                                {{-- Mengakses relasi rw untuk nomor_rw --}}
                                <td class="px-4 py-2">RW {{ sprintf('%02d', $rt->rw->nomor_rw) }}</td>
                                {{-- Kolom ketua_rw di tabel rws --}}
                                <td class="px-4 py-2">{{ $rt->rw->ketua_rw }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <section class="space-y-6">
            ...
            <p class="text-gray-700 leading-relaxed">
                ...
                Kelompok ini membina para petani, mengembangkan teknik budidaya, serta membantu dalam memasarkan hasil
                panen.
            </p>
            <p class="text-gray-700 leading-relaxed">
        ...
        <section class="space-y-6">
            ...
            <div class="overflow-x-auto">
                <table class="w-full bg-white rounded-lg shadow">
                    ...
                    <tbody>
                        @foreach ([['Tatang', 'Kepala Desa'], ['Endang Mulyana', 'Sekretaris Desa'], ['Andi Suryana', 'Kaur Pemerintahan'], ['Tedi', 'Kaur Kesejahteraan'], ['Yayat Suheryat', 'Kaur Keuangan'], ['Ajat Sudrajat', 'Kaur Umum'], ['Wawan Setiawan', 'Kasi Pelayanan'], ['Winda Heryana', 'Kasi Pemerintahan'], ['Yudi', 'Kasi Kesejahteraan'], ['Asep Saepulloh', 'Kadus 1'], ['Entang', 'Kadus 2']] as $index => $item)
